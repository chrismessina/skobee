...
class Test::Unit::TestCase
  ...
  def assert_valid_markup(fragment=@response.body)
    #MGS- check that the env var we set in the rake task is set before we attempt to validate
    return if ENV['VALIDATE_XHTML'] != 'true'
    ...
    if !markup_is_valid
      ...
      File.open filename, 'w+' do |f| Marshal.dump fragment , f end
      p "***  Storing invalid xhtml here: #{filename} for review.  ***"
    end
