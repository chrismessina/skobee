...
#MGS- begin custom
        ...
        #MGS- customized; don't encode ROLE, if address is for the organizer
        #if role.length > 0 #&& role != 'REQ-PARTICIPANT'
        if role.length > 0 && name != 'ORGANIZER'
          params['ROLE'] = Vpim::encode_paramtext(role)
        end

        #MGS- always default to inidividual; don't encode CUTYPE, if address is for the organizer
        if name != 'ORGANIZER'
          params['CUTYPE'] = 'INDIVIDUAL'
        end

        # FIXME - default value is different for non-vEvent
        #MGS- customized from:   if partstat.length > 0 && partstat != 'NEEDS-ACTION'
        # Not sure why they weren't allowing NEEDS-ACTION through, maybe I'm misreading the spec, but
        # I think it should be allowed...
        #MGS- don't show PARTSTAT for the ORGANIZER
        if partstat.length > 0 && name != 'ORGANIZER' #&& partstat != 'NEEDS-ACTION'
          params['PARTSTAT'] = Vpim::encode_paramtext(partstat)
        end

        #MGS- don't show RSVP for the organizer
        if rsvp && name != 'ORGANIZER'
          params['RSVP'] = 'true'
        end
#MGS- end custom
